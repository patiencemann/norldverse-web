<template>
    <div>
        ...
        <div v-for="(category, index) in docs" :key="category.id" class="dark:border-none dark:ring-1 dark:ring-gray-800 global-footer dark:bg-gray-800/50">
            <div v-if="index != 0" class="featured-subtitle global-subtitle">
                <div class="global-subtitle-section-title capitalize font-calistoga mt-3 dark:text-gray-200">
                   {{ category.category.name }}
                </div>
            </div>
            <div class="mt-3 py-14">
                <div class="loop-wrap" v-if="category.blogs">
                    <article class="item is-loop is-image" v-for="doc in category.blogs" :key="doc.id">
                        <div :class="'item-image global-image global-image-orientation global-radius is-landscape bg-gray-300'">
                            ...
                            <div class="flex items-center justify-center">
                                ...
                                <div class="absolute flex justify-end right-5 bottom-5">
                                    <div class="flex items-center md:m-0 md:justify-center">
                                        <ul class="flex flex-col items-start space-y-4 md:flex-row md:space-y-0 md:space-x-4">
                                            <li v-if="auth" class="flex font-calistoga text-[14px] shadow-sm leading-none text-gray-900 bg-white rounded-lg">
                                            ...
                                            <li class="flex font-calistoga text-[14px] shadow-sm leading-none text-gray-900 bg-white rounded-lg">
                                                <button class="flex items-center px-3 py-2 space-x-2 bg-gray-200 border border-gray-400 rounded-l-lg hover:bg-gray-300 focus:bg-gray-300 focus:outline-none">
                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                                                        <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z" />
                                                        <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                                    </svg>
                                                </button>
                                                <a class="flex items-center -ml-px px-3 py-2 font-semibold border border-gray-400 rounded-r-lg" href="#_">{{ doc.views }}</a>
                                            </li>
                                        </ul>
                                    </div>
                                </div>
    ...
    export default {
        ...
        methods: {
            async getDocs(tag) {
                this.isLoading = true;
                this.docs = [];
                let response = await axios.get(`/api/public/docs?categorize=true&&tag=${tag}`);
